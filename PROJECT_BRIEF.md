...
- *2026-08-03* — Recalibrated the `annotate_cells.py` low-confidence flag. The old fixed `margin<0.05` cutoff flagged 59-84% of clusters (uselessly high); replaced it with an adaptive rule (flag if best correlation below an absolute floor, OR if the best-vs-second margin is in the low tail *for that run* AND the three metrics Pearson/Spearman/cosine don't all agree on the pick — the consensus signal was previously computed but unused). Flag rate dropped to 16-29% across all runs, with the remaining flags confirmed to be genuinely ambiguous near-duplicate subtypes. Recalibrated from the retained per-cluster metrics without re-running any embedding.
- *2026-08-03* — Rechecked and **corrected the "V1↑WNT" claim**: it was never generated by a committed script (an ad-hoc read), so built the reproducible V1-vs-V2 per-pathway signaling-diff (sender/receiver separate, Mann-Whitney + FDR + effect size, on both the CP/MZ and subplate-only cell populations). WNT is actually **higher in V2**, not V1 — consistently across sender/receiver and both populations, but with a *small* effect size (the large cell counts make it statistically significant regardless). The "V2↑ECM/guidance" half of the original contrast holds (SEMA3/EPHB are among the top V2-higher pathways); the "V1↑WNT/FGF" half was wrong in direction and should not be cited.
- *2026-08-03* — Ran the ground-truth spatial-method evaluation, **reframed from areal-boundary to cortical-layer recovery**: because V1 and V2 are physically separate dissection pieces in this section (no contiguous areal boundary to recover), the eval instead scores how well each spatial method (Banksy, STAGATE) recovers cortical *layer* structure against the ground-truth `layer` labels. STAGATE recovers layers better than Banksy (higher layer purity and V-measure in every region), consistent with the earlier "STAGATE more layer-aligned" finding. This addresses methodological question #2 for this section's data.
- *2026-08-03* — GW34 persistence (longitudinal extension) **deferred/blocked**: the `gw34_umb5900_ba17.h5ad` data file is not on Explorer and no GW34 pipeline or cross-taxonomy harmonization exists. To be scoped once the file is acquired.
- *2026-08-17* — GW34 data blocker resolved (files located locally, from Zenodo, and the BA17 file staged to Explorer) and feasibility inspected: same 300-gene panel as GW20 (ENVI transfers directly), 38,530 area-labeled V1/V2 cells (A-V1 25.8K / B-V2 12.8K, same separate-dissection-piece structure). Two caveats found — GW20↔GW34 share only 5/31 H2 cell types (harmonize at the broad H1 level instead), and GW34 layers are coarse (cp/mz only, no subplate). Decisions for the composition/niche comparison: split EN-IT into DL/UL (GW20 derived from H2 layer labels), and keep GW20's ~15% progenitor/migrating cells (absent at mature GW34) as an "Other-progenitor" bucket rather than dropping them.
- *2026-08-20* — **GW34 persistence arm complete** (full scope: signaling + composition + spatial-domain niches; best-performing pipeline = STAGATE domains + native-H1 annotation, on a fresh GW34 ENVI→COMMOT substrate mirroring GW20). Headline result: **the compositional V1/V2 identity persists, but the V1-vs-V2 signaling environment largely does not.** (1) Composition — the layer-fated EN-IT bias is the most stable V1/V2 signal across development: EN-IT-UL stays V1-enriched and EN-IT-DL stays V2-enriched at *both* timepoints (V1-vs-V2 composition signature Spearman GW20-vs-GW34 = 0.57), overlaid on the expected maturation shift (excitatory/progenitor populations fall; interneurons/astrocytes/glia rise). (2) Signaling — the per-pathway V1-vs-V2 contrast correlates only weakly across timepoints (Spearman 0.21; ~38% of pathway-directions persist, ~29% reverse), i.e. the signaling niches substantially rewire with maturation. (3) WNT specifically does **not** persist — it reverses from marginally-V2 at GW20 to marginally-V1 at GW34 (both small effects), reinforcing that there is no stable directional WNT V1/V2 difference to cite. (4) ~half (8/16) of GW34 spatial-domain niches mutual-best-match a GW20 niche (several at cosine > 0.9). Reusable-by-args build; only new code = `build_gw34_envi_input.py`, `harmonize_h1.py`, `persistence_compare.py`, plus a `--commot-dir`/empty-population patch to `signaling_diff.py`.
- *2026-08-20* — **Productionized the project.** (1) Reorganized `scripts/` from flat files into stage folders (`envi/annotation/domains/commot/niches/gw34/viz/`) and extracted the pure helpers into an installable `neurospatial/` package (renamed from `src/`), removing the fragile `sys.path` sibling-imports; verified behavior-preserving (a stage rerun reproduced exact pre-reorg numbers). (2) Captured the 5 cluster conda envs as `envs/*.yml` (previously only in comments). (3) Added per-stage Docker images (`dockerfiles/`; 4 GPU + CPU analysis + banksy). (4) Built a Nextflow DSL2 pipeline (`main.nf` + `modules/`) with `GW20_2X2`/`GW34_PERSISTENCE` entry workflows and `standard`/`slurm`/`awsbatch`/`test` profiles; all hardcoded `/scratch` paths parameterized; both stub runs pass (GW20 22 processes, GW34 11). (5) Added pytest unit tests for the shared helpers (32 passing). (6) Rewrote the README with install/run instructions and a Mermaid DAG. AWS Batch profile is ready-to-run but untested (needs the user's ECR/queue). Committed across `d254e16` (reorg) → `3c86a97` (README); not yet pushed.
